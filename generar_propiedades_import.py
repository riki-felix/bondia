...
Maps NOMBRE to titulo (not nombre) as per propiedades_schema.sql
"""
import pandas as pd
import re
...
OUTPUT_SQL = "propiedades_import.sql"
...
def main():
	...
	with open(OUTPUT_SQL, 'w', encoding='utf-8') as f:
		...
		for idx, row in df.iterrows():
			...
			estado = escape_sql_string(row.get('ESTADO', ''))
			...
			f.write(f"-- Property: {titulo}\n")
			
			# Build INSERT statement
			f.write("INSERT INTO public.propiedades (\n")
			f.write("  id, titulo, estado, fecha_inicio, pago,\n")
			f.write("  aportacion, retribucion, retencion, ingreso_banco, efectivo,\n")
			f.write("  jasp_10_percent, ja, transfe, fecha_compra, fecha_venta\n")
			f.write(") VALUES (\n")
			f.write(f"  {format_sql_value(prop_id)}, {format_sql_value(titulo)}, {format_sql_value(estado)}, ")
			...
			f.write(f"{format_sql_value(fecha_compra)}, {format_sql_value(fecha_venta)}\n")
			f.write(")\n")
