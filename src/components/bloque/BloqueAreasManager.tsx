// src/components/bloque/BloqueAreasManager.tsx
...
interface BloqueAreasManagerProps {
  config: BloqueConfig;
  initialAreas: BloqueArea[];
  initialAssignments: BloqueAreaCategoria[];
...
export default function BloqueAreasManager({
  config,
  initialAreas,
  initialAssignments,
  gastoCategorias,
  ingresoCategorias,
  activoCategorias,
}: BloqueAreasManagerProps) {
  const [areas, setAreas] = useState<BloqueArea[]>(initialAreas);
  const [assignments, setAssignments] = useState<BloqueAreaCategoria[]>(initialAssignments);
  const [expandedArea, setExpandedArea] = useState<string | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<{ id: string; name: string } | null>(null);
  const [selectedTipo, setSelectedTipo] = useState<CatTipo>("gasto");

  const catMap: Record<CatTipo, BloqueCategoria[]> = {
  ...
  const addArea = useCallback(async () => {
    try {
      const res = await fetch(config.endpoints.createArea, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        try { throw new Error(JSON.parse(text).error); } catch { throw new Error(text || `Error ${res.status}`); }
      }
  ...
  const updateName = useCallback(
    async (id: string, nombre: unknown) => {
      ...
      try {
        const res = await fetch(config.endpoints.updateArea, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const deleteArea = useCallback(async (id: string) => {
    try {
      const res = await fetch(config.endpoints.deleteArea, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const toggleCategory = useCallback(
    async (areaId: string, tipo: CatTipo, categoriaId: string, checked: boolean) => {
      ...
      try {
        const res = await fetch(config.endpoints.syncAreaCategorias, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
