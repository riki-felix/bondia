// src/components/casa/CasaControlDashboard.tsx
...
interface Props {
  gastos: CasaGasto[];
  gastosOverrides: CasaOverride[];
  ingresos: CasaIngreso[];
  ingresosOverrides: CasaOverride[];
  ejercicio: number;
...
export default function CasaControlDashboard({
  gastos,
  gastosOverrides,
  ingresos,
  ingresosOverrides,
  ...
  // Gather all future payments starting from current month
  // Exclude variable and puntual — only recurring frequencies
  ...
  for (const g of gastos) {
    ...
    for (let m = mesActual; m <= 12; m++) {
      ...
      if (v != null && v > 0) {
        // Skip current month items that already "passed"
        // (we include all from current month forward)
        upcoming.push({ concepto: g.concepto, mes: m, importe: v });
      }
    }
  }

  // Sort by month proximity, then by amount desc
  upcoming.sort((a, b) => a.mes - b.mes || b.importe - a.importe);
  const next5 = upcoming.slice(0, 5);

  // ── Widget 4: Calendario fiscal ─────────────────────────────
  // Find "Finanzas" area and its assigned gasto category IDs
  ...
  // Aggregate totals per concepto — annual and current month
...
function RankingVidaDiariaCard({
  rankingAnual,
  rankingMes,
  mesLabel,
  barColors,
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        <CardTitle className="text-sm font-medium">Ranking Vida Diaria</CardTitle>
      ...
      <CardContent>
        {rankingAnual.length === 0 && rankingMes.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            Crea un área «Vida Diaria» y asigna categorías de gasto para ver el ranking.
          </p>
        ) : ranking.length === 0 ? (
