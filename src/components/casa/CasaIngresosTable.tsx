// src/components/casa/CasaIngresosTable.tsx
...
interface CasaIngresosTableProps {
  initialData: CasaIngreso[];
...
export default function CasaIngresosTable({
  initialData,
  initialOverrides,
  categorias,
  initialYear,
  areas = [],
  areaAssignments = [],
}: CasaIngresosTableProps) {
  const [rows, setRows] = useState<CasaIngreso[]>(initialData);
  ...
  const addRow = useCallback(async () => {
    try {
      const res = await fetch("/.netlify/functions/createCasaIngreso", {
        method: "POST",
  ...
  const updateField = useCallback(
    async (id: string, field: string, value: unknown) => {
      ...
      try {
        const res = await fetch("/.netlify/functions/updateCasaIngreso", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const deleteRow = useCallback(async (id: string) => {
    try {
      const res = await fetch("/.netlify/functions/deleteCasaIngreso", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  function rowTotal(row: CasaIngreso): number {
    let sum = 0;
    for (let m = 1; m <= 12; m++) {
