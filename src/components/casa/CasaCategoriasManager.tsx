// src/components/casa/CasaCategoriasManager.tsx
...
import { type CasaCategoria } from "@/lib/casaTypes";
...
interface CasaCategoriasManagerProps {
...
function CategoriaTable({
  ...
  const [rows, setRows] = useState<CasaCategoria[]>(initialCategorias);
  const [deleteTarget, setDeleteTarget] = useState<{ id: string; name: string } | null>(null);

  const addRow = useCallback(async () => {
    try {
      const res = await fetch("/.netlify/functions/createCasaCategoria", {
        method: "POST",
  ...
  const updateName = useCallback(
    async (id: string, nombre: unknown) => {
      ...
      try {
        const res = await fetch("/.netlify/functions/updateCasaCategoria", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const deleteRow = useCallback(
    async (id: string) => {
      try {
        const res = await fetch("/.netlify/functions/deleteCasaCategoria", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
...
export default function CasaCategoriasManager({
  gastoCategorias,
  ingresoCategorias,
  activoCategorias,
}: CasaCategoriasManagerProps) {
  return (
